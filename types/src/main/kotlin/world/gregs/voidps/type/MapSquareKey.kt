 ...
 */
@JvmInline
value class MapSquareKey(val id: Int) {//MapSquareKey
    ...
    fun copy(x: Int = this.x, y: Int = this.y) = MapSquareKey(x, y)
    ...
    fun add(point: MapSquareKey) = add(point.x, point.y)
    ...
    fun toRectangle(radius: Int) = Bounds(minus(radius, radius).tile, (radius * 2 + 1) * 64, (radius * 2 + 1) * 64)
    fun toRectangle(width: Int = 1, height: Int = 1) = Bounds(tile, width * 64, height * 64)
    ...
    companion object {
        ...
        val EMPTY = MapSquareKey(0, 0)
    }
}

fun MapSquareKey.equals(x: Int, y: Int) = this.x == x && this.y == y