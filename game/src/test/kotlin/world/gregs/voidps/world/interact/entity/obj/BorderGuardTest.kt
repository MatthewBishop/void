...
internal class BorderGuardTest : WorldTest() {
    ...
    fun `Can walk west through a vertical border`() {
        val player = createPlayer("player", CoordGrid(3112, 3420))
        tick()

        handler.validate(player, Walk(3106, 3421))
        tick(8)

        assertEquals(CoordGrid(3106, 3421), player.tile)
    }

    @Test
    fun `Can walk east through a vertical border`() {
        val player = createPlayer("player", CoordGrid(3106, 3421))
        tick()

        handler.validate(player, Walk(3112, 3420))
        tick(8)

        assertEquals(CoordGrid(3112, 3420), player.tile)
    }

    @Test
    fun `Can walk south through a horizontal border`() {
        val player = createPlayer("player", CoordGrid(3292, 3387))
        tick()

        handler.validate(player, Walk(3293, 3383))
        tick(6)

        assertEquals(CoordGrid(3293, 3383), player.tile)
    }

    @Test
    fun `Can walk north through a horizontal border`() {
        val player = createPlayer("player", CoordGrid(3293, 3383))
        tick()

        handler.validate(player, Walk(3292, 3387))
        tick(6)

        assertEquals(CoordGrid(3292, 3387), player.tile)
    }
